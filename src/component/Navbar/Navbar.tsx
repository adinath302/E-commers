...
const Navbar = () => {
  ...
  useEffect(() => {
    setIsScreenLarge(!isScreenSmall);
  }, [isScreenSmall]);
  ...
  return (
    <div className="py-3 relative bg-gray-200 items-center justify-between px-4 gap-10 flex">
      ...
      <div>
        <ul className="flex gap-6 items-center flex-1 hidden sm:flex">
          {NAV_LINKS.map((link) => (
...
const MobailSidebar = ({ isOpen, onClose }: Mobailview) => {
  return (
    <div
      className={` ${isOpen && "translate-x-[400px]"} fixed top-0 left-[-400px] z-50 bg-gray-100 w-50 h-screen transition-all duration-600 `}
    >
      ...
      <ul className=" flex flex-col gap-8">
        <div className="flex-none mx-auto font-bold text-purple-500 mt-4">
          E-commers
        </div>
        {NAV_LINKS.map((link) => (
          <li
